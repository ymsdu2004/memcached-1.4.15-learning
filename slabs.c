...
typedef struct {
	...
	/* 每个slab包含的chunk数(即perslab表示每个slab切分成多少个chunk) */
    unsigned int perslab;   /* how many items per slab */

	/* 回收的item链表, 从某个slabclass分配出去一个item, 当item回收的时候,
	 * 不是把这item使用的内存交还给slab, 而是让这个item挂在slots链表的尾部
	 */
	 ...
	 * memcached就分配一个新的slab, 所以slabclass可以拥有多个slab, 这些slab
	 * 就是通过slab_list指针数组来管理的
	 */
